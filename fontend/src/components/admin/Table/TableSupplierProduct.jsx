...
import { removeSupplierProduct } from "../../../api/SupplierProduct";

const TableSupplierProduct = () => {
  const token = useCarStore((state) => state.token);
  const getSuppliersProduct = useCarStore((state) => state.getSuppliersProduct);
  const suppliersproducts = useCarStore((state) => state.suppliersproducts);
  ...
  // ປັບປຸງການດຶງ ແລະ ປັບໂຄ່ງຂໍ້ມູນ
  const supplierData = useMemo(() => {
    const rawData = suppliersproducts || [];
    console.log("rawData", rawData);
    // ຖ້າຂໍ້ມູນເປັນ nested structure ຈາກ backend ໃຫ້ flatten
    return rawData.map(item => ({
      id: item.id,
      // ຂໍ້ມູນ supplier
      companyName: item.supplier?.companyName || item.companyName || '',
      contactName: item.supplier?.contactName || item.contactName || '',
      email: item.supplier?.email || item.email || '',
      phone: item.supplier?.phone || item.phone || '',
      address: item.supplier?.address || item.address || '',
      // ຂໍ້ມູນ car (ຖ້າມີ)
      carBrand: item.car?.brandAndModels?.BrandCars?.name || '',
      carModel: item.car?.brandAndModels?.name || '',
      carType: item.car?.typecar?.name || '',
      price: item.price || 0,
      isActive: item.isActive,
      createdAt: item.createdAt,
      updatedAt: item.updatedAt,
      // ເກັບ raw data ໄວ້ສຳລັບ modal
      _raw: item
    }));
  }, [suppliersproducts]);

  // ປັບປຸງຟັງຊັນການຄົ້ນຫາ
  const filteredsuppliersproducts = useMemo(() => {
  ...
  const currentProducts = filteredsuppliersproducts.slice(indexOfFirst, indexOfLast);
  const totalPages = Math.ceil(filteredsuppliersproducts.length / productsPerPage);
  ...
  const handleRemove = async (id) => {
    ...
    try {
      const res = await removeSupplierProduct(token, id);
      toast.success(
        <span className="font-phetsarath">ລົບ {res.data.name || 'ລາຍການ'} ສຳເລັດ</span>
      );
      getSuppliersProduct();
    } catch (err) {
      console.log(err);
  ...
  const formatPrice = (price) => {
    if (!price || price === 0) return "ບໍ່ລະບຸ";
    return new Intl.NumberFormat('lo-LA').format(price) + " ກີບ";
  };

  if (!suppliersproducts) {
    return (
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center font-notosanslao justify-between space-y-3 sm:space-y-0">
          <div>
            <h2 className="text-lg font-semibold text-gray-800 font-phetsarath">
              ລາຍການຜູ້ສະໜອງ-ສິນຄ້າ
            </h2>
            <p className="text-sm text-gray-600 mt-1">
              ພົບ {filteredsuppliersproducts.length} ລາຍການທັງໝົດ
            </p>
          </div>
          ...
          <div className="flex space-x-2">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="ຄົ້ນຫາບໍລິສັດ, ຜູ້ຕິດຕໍ່, ອີເມວ..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="pl-9 pr-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm font-phetsarath w-64"
              />
            </div>
            {searchTerm && (
              <button
            ...
            {currentProducts.length > 0 ? (
              currentProducts.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900 font-phetsarath">
                      {item.contactName || '-'}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full font-phetsarath ${
                      item.isActive 
                  ...
                  >
                    <div className="flex justify-center space-x-2">
                      ...
                      <Link
                        to={`/admin/edit-suppliersproducts/${item.id}`}
      ...
      {/* Pagination */}
      {filteredsuppliersproducts.length > productsPerPage && (
        <div className="px-6 py-4 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-700 font-phetsarath">
              ສະແດງ {indexOfFirst + 1} ຫາ{" "}
              {Math.min(indexOfLast, filteredsuppliersproducts.length)} ຈາກທັງໝົດ{" "}
              {filteredsuppliersproducts.length} ລາຍການ
            </div>
          ...
          <div className="bg-white rounded-lg w-full max-w-2xl shadow-xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="px-6 py-4 space-y-6">
              ...
              <div className="border-b border-gray-200 pb-4">
                <h4 className="text-md font-semibold text-gray-800 font-phetsarath mb-3">
                  ຂໍ້ມູນຜູ້ສະໜອງ
                </h4>
              ...
              {/* ຂໍ້ມູນລົດ */}
              {(selectedProduct.carBrand || selectedProduct.carModel || selectedProduct.carType) && (
                <div className="border-b border-gray-200 pb-4">
                  <h4 className="text-md font-semibold text-gray-800 font-phetsarath mb-3">
                    ຂໍ້ມູນລົດ
                  </h4>
                  ...
                  <div>
                    ...
                    <p className="mt-1">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full font-phetsarath ${
                        ...
                        {selectedProduct.isActive ? 'ເປີດໃຊ້' : 'ປິດໃຊ້'}
                      </span>
                    </p>
                  </div>
                  <div>
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end space-x-3 sticky bottom-0 bg-white">
              <Link
                to={`/admin/edit-suppliersproducts/${selectedProduct.id}`}
...
export default TableSupplierProduct;