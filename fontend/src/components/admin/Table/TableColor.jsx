...
import { removeColor } from "../../../api/Color";
import { 
  Search, 
  X, 
  Edit, 
...
const TableColor = () => {
  ...
  const filteredcolors = useMemo(() => {
    ...
    return colors.filter((colors) =>
      colors.name?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [colors, searchTerm]);
  ...
  const currentProducts = filteredcolors.slice(indexOfFirst, indexOfLast);
  const totalPages = Math.ceil(filteredcolors.length / productsPerPage);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              ພົບ {filteredcolors.length} ລາຍການທັງໝົດ
            </p>
          </div>
      ...
      {/* Pagination */}
      {filteredcolors.length > productsPerPage && (
        <div className="px-6 py-4 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-700 font-phetsarath">
              ສະແດງ {indexOfFirst + 1} ຫາ {Math.min(indexOfLast, filteredcolors.length)} ຈາກທັງໝົດ {filteredcolors.length} ລາຍການ
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-md shadow-xl">
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
              <Link
                to={`/admin/edit-colors/${selectedProduct.id}`}
