...
import { readColor, updateColor } from "../../../api/Color";
const inirialState = {
  name: "",
};
const EditColor = () => {
  ...
  useEffect(() => {
    // getSupplier();
    fetchColor(token, id, form);
  }, []);

  const fetchColor = async (id, form) => {
    try {
      const res = await readColor(id, form);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateColor(token, id, form);
      // console.log(res);
      toast.success(<p className="font-phetsarath">ອັບເດດສີລົດສຳເລັດ</p>);
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  placeholder="ຊື່ສີລົດ ເຊັ່ນ: (ດຳ, ແດງ, ຟ້າ.....)"
                  type="text"
                  maxLength={191}
                  name="name"
                  value={form.name}
