...
const EditSupplier = () => {
  ...
  const fetchSupplier = async (supplierId) => {
    try {
      const res = await readSupplier(token, supplierId);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      // ແກ້ໄຂຕໍ່ນີ້: ເພີ່ມ id ເປັນ parameter ທີ່ 2
      const res = await updateSupplier(token, id, form);
      
      toast.success(
        <span className="font-phetsarath">
          ອັບເດດ {res.data.companyName} ສຳເລັດ
        </span>
      );
      getSupplier();
      
      // ກັບໄປໜ້າລາຍການຫຼັງອັບເດດສຳເລັດ
      navigate("/admin/suppliers");
    } catch (err) {
      const errMessage = err.response?.data?.message || "ເກີດຂໍ້ຜິດພາດ";
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="flex gap-4">
              <button
                type="button"
                onClick={() => navigate("/admin/suppliers")}
                className="px-6 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 transition-colors font-notosanslao"
              >
                ຍົກເລີກ
              </button>
              ...
              <button
                type="submit"
                disabled={isLoading || !form.companyName?.trim() || !form.email?.trim() || !form.phone?.trim()}
                className="bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 text-white px-6 py-2 rounded-md font-medium transition-colors flex items-center justify-center space-x-2 disabled:cursor-not-allowed font-notosanslao"
              >
                {isLoading ? (
                  <>
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                    <span>ກຳລັງອັບເດດ...</span>
                  </>
                ) : (
                  <>
