...
import { saveColor } from "../../../api/Color";
...
const FromColor = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name.trim()) {
      toast.error(<span className="font-phetsarath">ກະລຸນາປ້ອນຊື່ສີລົດ</span>);
      return;
    }

    setIsLoading(true);
    try {
      const res = await saveColor(token, { name });
